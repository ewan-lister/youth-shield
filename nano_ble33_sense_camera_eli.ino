...
/* Includes ---------------------------------------------------------------- */
#include <Age_Estimation_2.0_inferencing.h>
...
// Below is the code we use to set up the numberpad

#include <Keypad.h>
#define OV2640_MINI_2MP
const byte ROWS = 4;
const byte COLS = 4;
const int acscii_base = 48;
...
void loop()
{
    ...
    while(stop_inferencing == false) {
        ...
        for (size_t ix = 0; ix < EI_CLASSIFIER_LABEL_COUNT; ix++) {
            ei_printf("Printing Model Age Estimate Value\n");
            ei_printf("    %s: %.5f\n", result.classification[ix].label,
                                        result.classification[ix].value);

            //This might be the right area 
            //float confidence = -18; // model output of how confident it is on the age range
            //float range = ;	 // our parameter for adjusting based on confidence
            float ageEstimate = result.classification[ix].value;	// model estimate of the age
            int cutoffAge = 14;	// user input of cutoff age
            float ageAdjustment = -1;

            // calculate the whether to open or close the lock

            ei_printf("You're trying to open the lock with the",cutoffAge);

            //ageAdjustment = cutoffAge - (ageEstimate - ((1 - confidence) % range)); 
            ageAdjustment = ageEstimate - 18 - cutoffAge; 

            if (ageAdjustment > 0) {
              ei_printf("Age Accepted Lock Opening\n");
	            digitalWrite(12, HIGH);
              delay(7000);
              digitalWrite(12, LOW); 
            } else {
              ei_printf("You're Too Young! Try again or start waiting!\n");
            }           
