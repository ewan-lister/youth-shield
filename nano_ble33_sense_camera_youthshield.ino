    ...
    while(stop_inferencing == false) {
        ...
        for (size_t ix = 0; ix < EI_CLASSIFIER_LABEL_COUNT; ix++) {
            ...
            ei_printf("    %s: %.5f\n", result.classification[ix].label,
                                        result.classification[ix].value - 10);

            //This is the area modified by YouthShield Group 
     
            float ageEstimate = result.classification[ix].value;	// model estimate of the age
            int cutoffAge = 25;	// user input of cutoff age
            float ageAdjustment = -1;

            // calculate the whether to open or close the lock

            ei_printf("You're trying to open the lock with the cutoff age of %i \n", cutoffAge);

            //ageAdjustment = cutoffAge - (ageEstimate - ((1 - confidence) % range)); 
            ageAdjustment = ageEstimate - 10 - cutoffAge; 
